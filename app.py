...
def main():
    ...
    for iv in intervals:
        ...
        thresholds[iv] = st.sidebar.number_input(
            ...
            min_value=0.0, max_value=1.0, value=float(DEFAULT_THRESHOLDS.get(iv, 0.04)), step=0.001, format="%.3f", key=f"th_{iv}"
        )
        deltas[iv] = st.sidebar.number_input(
            ...
            min_value=0.0, max_value=1.0, value=float(DEFAULT_DELTAS.get(iv, 0.1)),step=0.001, format="%.3f", key=f"dl_{iv}"
        )
